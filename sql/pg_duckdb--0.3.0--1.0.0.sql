...
CREATE TABLE duckdb.ducklake_metadata(key VARCHAR NOT NULL, value VARCHAR NOT NULL, scope VARCHAR, scope_id BIGINT);
INSERT INTO duckdb.ducklake_metadata VALUES
    ('version', '0.1'),
    ('created_by', 'pg_ducklake'),
    ('data_path', '/tmp/ducklake'),
    ('encrypted', 'false');

CREATE TABLE duckdb.ducklake_snapshot(
...
INSERT INTO duckdb.ducklake_snapshot VALUES (0, NOW(), 0, 1, 0);

CREATE TABLE duckdb.ducklake_snapshot_changes(
    snapshot_id BIGINT PRIMARY KEY,
    changes_made VARCHAR
);
INSERT INTO duckdb.ducklake_snapshot_changes VALUES (0, 'created_schema:"main"');

CREATE TABLE duckdb.ducklake_schema(
...
INSERT INTO duckdb.ducklake_schema VALUES (0, gen_random_uuid(), 0, NULL, 'main', 'main/', true);

CREATE TABLE duckdb.ducklake_table(
    table_id BIGINT,
    table_uuid UUID,
    begin_snapshot BIGINT,
    end_snapshot BIGINT,
...
CREATE TABLE duckdb.ducklake_view(
    view_id BIGINT,
    view_uuid UUID,
    begin_snapshot BIGINT,
    end_snapshot BIGINT,
...
CREATE TABLE duckdb.ducklake_tag(
    object_id BIGINT,
    begin_snapshot BIGINT,
    end_snapshot BIGINT,
    key VARCHAR,
...
CREATE TABLE duckdb.ducklake_column_tag(
    table_id BIGINT,
    column_id BIGINT,
    begin_snapshot BIGINT,
    end_snapshot BIGINT,
...
CREATE TABLE duckdb.ducklake_data_file(
...
CREATE TABLE duckdb.ducklake_file_column_statistics(
    data_file_id BIGINT,
    table_id BIGINT,
    column_id BIGINT,
...
CREATE TABLE duckdb.ducklake_delete_file(
...
CREATE TABLE duckdb.ducklake_column(
    column_id BIGINT,
    begin_snapshot BIGINT,
    end_snapshot BIGINT,
    table_id BIGINT,
...
CREATE TABLE duckdb.ducklake_table_stats(
    table_id BIGINT,
    record_count BIGINT,
    next_row_id BIGINT,
    file_size_bytes BIGINT
...
CREATE TABLE duckdb.ducklake_table_column_stats(
    table_id BIGINT,
    column_id BIGINT,
...
CREATE TABLE duckdb.ducklake_partition_info(
    partition_id BIGINT,
    table_id BIGINT,
    begin_snapshot BIGINT,
    end_snapshot BIGINT
...
CREATE TABLE duckdb.ducklake_partition_column(
    partition_id BIGINT,
    table_id BIGINT,
...
CREATE TABLE duckdb.ducklake_file_partition_value(
    data_file_id BIGINT PRIMARY KEY,
    table_id BIGINT,
    partition_key_index BIGINT,
...
CREATE TABLE duckdb.ducklake_files_scheduled_for_deletion(
    data_file_id BIGINT,
    path VARCHAR,
    path_is_relative BOOLEAN,
...
CREATE TABLE duckdb.ducklake_inlined_data_tables(
    table_id BIGINT,
    table_name VARCHAR,
    schema_version BIGINT
);

CREATE TABLE duckdb.ducklake_column_mapping(mapping_id BIGINT, table_id BIGINT, type VARCHAR);

CREATE TABLE duckdb.ducklake_name_mapping(
    mapping_id BIGINT,
    column_id BIGINT,
    source_name VARCHAR,
...
CREATE FUNCTION ducklake_handler(internal) RETURNS table_am_handler
    AS 'MODULE_PATHNAME' LANGUAGE C STRICT;

CREATE ACCESS METHOD ducklake TYPE TABLE HANDLER ducklake_handler;
