...
FindOption(List *options_list, const char *name) {
	foreach_ptr(DefElem, def, options_list) {
...
void
assert_has_required_options(List *options_list, FdwType type, Oid context) {
	for (const struct PGDuckDBFdwOption *opt = valid_options; opt->optname != NULL; ++opt) {
...
void
assert_has_no_motherduck_foreign_server() {
...
void
assert_options_valid_and_complete(List *options_list, FdwType type, Oid context) {
	...
	assert_has_required_options(options_list, type, context);
}
...
validate_motherduck_server_fdw(List *options_list, Oid context) {
	...
	assert_options_valid_and_complete(options_list, MD, context);
...
pgduckdb_fdw_validator(PG_FUNCTION_ARGS) {
	...
	if (catalog == ForeignDataWrapperRelationId) {
		...
		assert_options_valid_and_complete(options_list, server_type, catalog);
