...
PostgresReplacementScan(duckdb::ClientContext &context, duckdb::ReplacementScanInput &input,
	...
	if (scan_data.m_query_planner_info) {
		int i = 1;
		for (; i < scan_data.m_query_planner_info->simple_rel_array_size; i++) {
			if (relid == scan_data.m_query_planner_info->simple_rte_array[i]->relid) {
				break;
			}
		}
		node = scan_data.m_query_planner_info->simple_rel_array[i];
	...
	if ((nodePath != nullptr && (nodePath->pathtype == T_IndexScan))) {
