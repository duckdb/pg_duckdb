...
PostgresScanGlobalState::InitGlobalState(duckdb::TableFunctionInitInput &input) {
	...
	duckdb::map<duckdb::idx_t, duckdb::column_t> ordered_input_columns;
	...
	for (auto const &[columnIdx, outputIdx] : ordered_input_columns) {
		duckdb::TableFilter *column_filter = nullptr;
		if (table_filters) {
			auto column_filter_it = table_filters->filters.find(outputIdx);
			if (column_filter_it != table_filters->filters.end()) {
				column_filter = column_filter_it->second.get();
			}
		}
		m_read_columns_ids.push_back(duckdb::make_pair(columnIdx, outputIdx));
		m_column_filters[outputIdx] = column_filter;
	}
	...
	if (input.CanRemoveFilterColumns()) {
		for (duckdb::idx_t i = 0; i < input.projection_ids.size(); i++) {
			m_output_columns_ids.push_back(
			    duckdb::make_pair(input.projection_ids[i], input.column_ids[input.projection_ids[i]]));
		}
	} else {
		for (duckdb::idx_t i = 0; i < input.column_ids.size(); i++) {
			m_output_columns_ids.push_back(duckdb::make_pair(i, input.column_ids[i]));
		}
	}
}
