...
def test_autoinstall_known_extensions(pg: Postgres, cur: Cursor):
    ...
    assert (
        cur.sql(
            "SELECT * FROM duckdb.query($$ FROM duckdb_extensions() SELECT installed, loaded WHERE extension_name = 'mooncake' $$)"
        )
        == []
    )

    cur.sql("SELECT duckdb.install_extension('mooncake', 'community')")
    assert cur.sql(
        "SELECT * FROM duckdb.query($$ FROM duckdb_extensions() SELECT installed, loaded WHERE extension_name = 'mooncake' $$)"
    ) == (True, True)
    ...
    with pytest.raises(
        ...
        match=r'mooncake.duckdb_extension" not found.',
    ):
