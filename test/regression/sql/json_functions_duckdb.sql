-- <JSON_EXISTS>
-- Test 1: Path exists in a simple JSON object (using a json type)
SELECT json_exists('{"a": {"b": 1}}'::json, '$.a.b'); -- Expected: true

-- Test 2: Path does not exist in a simple JSON object (using JSONB)
...
SELECT json_extract(r['data'], '$.a.b.c') AS result FROM duckdb.query($$ SELECT '{"a": {"b": {"c": 42}}}'::json AS data $$) r; -- Expected: 42
...
-- Nested JSON Extraction
SELECT json_value('{"a": {"b": {"c": 42}}}', '$.a.b.c') AS result; -- Expected: 42

-- Non-existent Path
SELECT json_value('{"key": "value"}', '$.nonexistent') AS result; -- Expected: NULL
...
SELECT json_array_length('[1, 2, 3, 4, 5]', '$') AS array_length; -- Expected: 5

-- Test with a JSON object that doesn't contain an array at the path
SELECT json_array_length('{"not_an_array": {"key": "value"}}', '$.not_an_array') AS array_length; -- Expected: Error or NULL
...
SELECT json_contains('[1, 2, 3, 4]', '2') AS contains_numeric; -- Expected: TRUE

-- JSON object containing the needle as a value
SELECT json_contains('{"key1": "value1", "key2": 42}', '"value1"') AS contains_object_value; -- Expected: TRUE
...
SELECT json_structure('{"name": "John", "age": 30, "address": {"city": "New York", "zip": "10001"}}')
AS structure;
...
SELECT json_structure('{"data": [1, "string", {"key": "value"}]}')
AS structure;
...
SELECT json_type('{"name": "John", "age": 30, "isEmployed": true, "skills": ["SQL", "Python"]}')
AS top_level_type;
...
SELECT json_type('{"name": "John", "age": 30, "isEmployed": true, "skills": ["SQL", "Python"]}', ARRAY['name', 'age', 'isEmployed', 'skills'])
AS element_types;
...
-- <JSON_VALID>
-- Test 1: Valid JSON
SELECT json_valid('{"name": "John", "age": 30, "skills": ["SQL", "Python"]}') AS is_valid;
